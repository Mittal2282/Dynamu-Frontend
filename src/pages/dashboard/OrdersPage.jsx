    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs font-semibold text-white leading-tight line-clamp-1">
          {item.name}
          {variantBit && (
            <span className="font-medium text-slate-400"> · {variantBit}</span>
          )}
        </p>
        {instruct && (
          <p
      ...
      <div className="text-right shrink-0 leading-tight">
        <p className="text-[10px] text-slate-500 tabular-nums">×{item.quantity ?? 1}</p>
    ...
    <div className="space-y-1.5">
      <div className="flex items-center justify-between gap-1.5 min-h-[22px]">
        <div className="flex items-center gap-1.5 min-w-0 flex-1 overflow-hidden">
          ...
          <span className="text-[10px] text-slate-500 font-mono shrink-0">#{order.order_number}</span>
    ...
    <div className="flex flex-col gap-3 min-h-0">
      {orders.length > 0 && (
        <div className="flex flex-wrap items-center gap-2 gap-y-2">
          {showTableFilter && useTableSelect && (
            <label className="flex items-center gap-2 text-[11px] text-slate-500 shrink-0">
          ...
          <span className="text-[10px] text-slate-500 shrink-0">
            Auto 10s
            {lastRefresh && (
              <span className="text-slate-600"> · {lastRefresh.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</span>
            )}
          </span>
      ...
      {orders.length === 0 && (
        ...
        >
          ...
          <p className="text-slate-500 text-xs">Waiting for customers to place orders…</p>
          <button
            type="button"
            onClick={() => fetchOrders()}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:h-full md:min-h-0 md:grid-rows-1">
            {DASHBOARD_COLUMNS.map(({ key, label, color }) => {
              ...
              return (
                <div
                  key={key}
                  className="flex flex-col min-h-0 md:h-full md:min-h-0"
                >
                  <div
                  ...
                  >
                    <div className="flex items-center gap-1.5 min-w-0">
                      ...
                      <h2 className="text-[11px] font-bold uppercase tracking-wide truncate" style={{ color }}>
                        {label}
                      </h2>
                    </div>
                    <div
