...
function formatCoord(v: number | null | undefined, axis: "lat" | "lng"): string {
...
export default function LocationPanel() {
  ...
  const [synced_at, setSyncedAt]    = useState<string | null>(null);
  ...
  const initialRef = useRef<LocationForm | null>(null);
  ...
  useEffect(() => {
    ...
    getRestaurantLocation()
      .then((data: unknown) => {
        const d = data as {
          location?: { coordinates?: number[]; accuracy_m?: number; label?: string; synced_at?: string };
        ...
        const next: LocationForm = {
          ...
          label:      loc.label ?? "",
          radius_m:   d?.proximity_radius_m ?? 2000,
          enforce_proximity: d?.enforce_proximity ?? true,
        };
        setForm(next);
  ...
  const center    = hasCoords ? { lat: form.latitude as number, lng: form.longitude as number } : DEFAULT_CENTER;
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex items-start justify-between gap-4 px-5 pt-5 pb-3">
          ...
          >
            <span className="w-1.5 h-1.5 rounded-full" style={{ background: hasCoords ? "#4ade80" : "#f87171" }} />
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 px-5 pb-4">
          ...
          <Stat label="Accuracy" value={form.accuracy_m != null ? `±${Math.round(form.accuracy_m)} m` : "—"} />
        </div>
        ...
        >
          <div className="h-[340px] w-full">
            ...
            >
              ...
              {hasCoords && (
                <>
                  <Marker position={[form.latitude as number, form.longitude as number]} icon={markerIcon} />
                  <Circle
        ...
        <div className="px-5 py-4 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
          ...
          >
            {syncing ? (
              ...
              <><span>📡</span> Sync my current location</>
            )}
          </button>
        </div>
      </section>
      ...
      >
        <div>
          <h2 className="text-base font-bold" style={{ color: "var(--t-text)" }}>Customer Proximity</h2>
          <p className="text-xs mt-1" style={{ color: "var(--t-dim)" }}>
        ...
        >
          ...
          <span
            ...
            style={{
              ...
              boxShadow: allowAll ? "0 0 8px rgba(239,68,68,0.25)" : "0 0 10px rgba(34,197,94,0.4)",
            }}
          >
            <span
        ...
        <div>
          <div className="flex items-baseline justify-between mb-2">
            <label className="text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--t-dim)" }}>Radius</label>
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl px-3 py-2.5" style={{ background: "var(--t-float)", border: "1px solid var(--t-line)" }}>
      <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: "var(--t-dim)" }}>{label}</p>
      <p className="text-sm font-mono font-semibold mt-1 truncate" style={{ color: "var(--t-text)" }}>{value}</p>
    </div>
  );
}
