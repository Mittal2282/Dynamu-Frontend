...
export default function BulkUploadModal({ onClose, onSaved }: BulkUploadModalProps) {
  ...
  const handleSave = async () => {
    ...
    setError('');
    try {
      const res = await createBulkOrders(rows, date) as UploadResult;
      setResult(res);
      onSaved();
    } catch (err: unknown) {
                  ...
                  <tbody>
                    {preview.map((r, i) => (
                      <tr key={i} style={{ borderTop: '1px solid var(--t-line)' }}>
                        <td className="px-3 py-2 text-white">{String(r['Item Name'] ?? r['item_name'] ?? '—')}</td>
                        <td className="px-3 py-2" style={{ color: 'var(--t-dim)' }}>{String(r['Variant Name'] ?? r['variant_name'] ?? '—')}</td>
                        <td className="px-3 py-2" style={{ color: 'var(--t-dim)' }}>{String(r['Quantity'] ?? r['quantity'] ?? '1')}</td>
                        <td className="px-3 py-2 max-w-[160px] truncate" style={{ color: 'var(--t-dim)' }}>{String(r['Special Instructions'] ?? r['special_instructions'] ?? '')}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {result && (
            <div className="rounded-xl p-4 flex flex-col gap-2" style={{ background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.2)' }}>
              ...
              <p className="text-sm" style={{ color: 'var(--t-text)' }}>
                ...
                {result.failed && result.failed.length > 0 ? `, ${result.failed.length} skipped` : ''}.
              </p>
