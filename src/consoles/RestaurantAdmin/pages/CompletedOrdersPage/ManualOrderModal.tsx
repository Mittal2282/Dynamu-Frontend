...
export default function ManualOrderModal({ onClose, onSaved }: ManualOrderModalProps) {
  ...
  useEffect(() => {
    (getDashMenu() as Promise<MenuItem[]>).then(setMenuItems).catch(() => {});
    (getDashTables() as Promise<TableItem[]>).then(setTables).catch(() => {});
  }, []);
  ...
  const linePrice = (line: OrderLine): number => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError(error?.response?.data?.message ?? error.message ?? 'Failed to save order');
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-4">
          ...
          <div>
            ...
            <div className="rounded-xl overflow-hidden flex flex-col divide-y"
              style={{ border: '1px solid var(--t-line)', maxHeight: 240, overflowY: 'auto' } as React.CSSProperties}>
