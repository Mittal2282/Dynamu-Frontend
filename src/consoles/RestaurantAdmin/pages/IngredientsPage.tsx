...
function VegDot({ type = "veg", size = "sm" }: VegDotProps) {
  const cfg: Record<VegType, { border: string; dot: string }> = {
...
const VEG_LABELS: Record<VegType, string> = { veg: "Veg", nonveg: "Non-Veg", mixed: "Mixed" };
const VEG_COLORS: Record<VegType, string> = { veg: "#4ade80", nonveg: "#f87171", mixed: "#fbbf24" };
...
function getVegType(items_using: DishRef[] = []): VegType {
...
function IngredientCard({ ingredient, onToggle, saving }: IngredientCardProps) {
    ...
    <div>
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <VegDot type={vegType} />
          <span className="text-[11px] font-medium shrink-0 hidden sm:block" style={{ color: VEG_COLORS[vegType] }}>
            {VEG_LABELS[vegType]}
          </span>
        </div>
          ...
          <ul className="divide-y" style={{ borderColor: "var(--t-line)" }}>
            {items_using.map((dish) => (
              <li key={dish._id} className="flex items-center gap-2.5 px-3 py-2">
                ...
                {dish.price != null && (
                  <span className="text-[10px] font-bold shrink-0" style={{ color: "var(--t-accent)" }}>₹{dish.price}</span>
                )}
              </li>
            ))}
          </ul>
...
function StatChip({ label, value, color }: StatChipProps) {
  return (
    ...
    >
      ...
      <p className="text-[11px] font-medium truncate" style={{ color: "var(--t-dim)" }}>{label}</p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-3">
        {[80, 56, 48].map((w, i) => (
          <div key={i} className="flex-1 rounded-2xl px-4 py-3 space-y-2" style={{ background: "var(--t-surface)", border: "1px solid var(--t-line)" }}>
      ...
      {initialLoad ? (
        ...
        >
          ...
          <button
            ...
            style={{ background: "var(--t-accent-20)", color: "var(--t-accent)", border: "1px solid var(--t-accent-20)" }}
          >
            Retry
          </button>
        </div>
        ...
        >
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-3xl mb-4" style={{ background: "var(--t-float)" }}>🌿</div>
