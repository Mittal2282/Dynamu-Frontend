...
interface ItemHandlers {
  ...
  onToggleSpecial: (id: string) => Promise<void>;
  onToggleFeatured: (id: string) => Promise<void>;
  onEdit: (item: MenuItem) => void;
  onDelete: (id: string) => Promise<void>;
  onUpdateDiscount: (id: string, val: number) => Promise<void>;
  onUpdateVariants: (id: string, variants: Variant[]) => Promise<void>;
  saving: string | null;
}
...
function MenuItemCard({
  ...
  const commitVariant = (idx: number, field: 'price' | 'discount', rawValue: string) => {
    const val = Math.min(
    ...
    const current = (item.variants?.[idx][key] as number) ?? 0;
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2.5 p-3 flex-1">
                    ...
                    <div className="flex items-center justify-end gap-1.5">
                      {isDraftingDisc ? (
                        <input
                          ...
                          onChange={(e) => setVariantDraft((d) => d ? { ...d, value: e.target.value } : d)}
                          onBlur={() => variantDraft && commitVariant(idx, "discount", variantDraft.value)}
                          onKeyDown={(e) => {
                            if (e.key === "Enter" && variantDraft) commitVariant(idx, "discount", variantDraft.value);
                      ...
                      {isDraftingPrice ? (
                        <input
                          ...
                          value={variantDraft.value}
                          onChange={(e) => setVariantDraft((d) => d ? { ...d, value: e.target.value } : d)}
                          onBlur={() => variantDraft && commitVariant(idx, "price", variantDraft.value)}
                          onKeyDown={(e) => {
                            if (e.key === "Enter" && variantDraft) commitVariant(idx, "price", variantDraft.value);
                            if (e.key === "Escape") setVariantDraft(null);
                          }}
        ...
        {/* Availability row */}
        {blockedByIngredient ? (
          <div className="flex items-center gap-2 px-2.5 py-1.5 rounded-xl" style={{ background: "rgba(251,146,60,0.07)", border: "1px solid rgba(251,146,60,0.2)" }}>
            <span className="text-sm">🚫</span>
            <span className="text-[11px] font-semibold text-orange-400 leading-tight">Blocked · ingredient off</span>
          </div>
        ) : (
          <div
      ...
      {confirmDelete && (
        ...
        >
          ...
          <div className="flex gap-2 w-full">
            <button onClick={() => setConfirmDelete(false)} className="flex-1 py-1.5 text-xs font-semibold rounded-lg border border-white/15 text-slate-300 hover:bg-white/10 transition-colors">
              Cancel
            </button>
            <button
...
function CategorySection({ category, items, handlers, onAddToCategory }: CategorySectionProps) {
...
function MenuItemSkeleton() {
  return (
    <div className="rounded-2xl overflow-hidden border flex flex-col" style={{ background: "var(--t-surface)", borderColor: "var(--t-line)" }}>
...
function MenuCategorySkeleton({ itemCount }: { itemCount: number }) {
  return (
    <div className="space-y-3">
...
export default function MenuManagePage() {
  ...
  useEffect(() => {
    getDashCategories().then(setCategories as (cats: unknown) => void).catch(() => {});
  }, []);
  ...
  const handleUpdateDiscount = async (itemId: string, value: number) => {
    setSaving(itemId);
    try {
  ...
  const handleUpdateVariants = async (itemId: string, updatedVariants: Variant[]) => {
    setSaving(itemId);
    try {
  ...
  /* ── Derived data ── */
  const grouped = (items as MenuItem[]).reduce<Record<string, MenuItem[]>>((acc, item) => {
  ...
  const availableCount = (items as MenuItem[]).filter((i) => i.is_available).length;
    ...
    <div className="space-y-7">
      ...
      {items.length === 0 && !loading && (
        ...
        >
          ...
          <p className="text-white font-semibold">{isFiltering ? "No items match your filters" : "Your menu is empty"}</p>
          <p className="text-slate-300 text-sm max-w-xs">
            {isFiltering ? "Try adjusting your search or filters." : "Add your first dish to get started."}
          </p>
          {isFiltering ? (
            <button
              onClick={() => { setSearchQuery(""); setCategoryFilter(""); setAvailFilter("all"); }}
      ...
      <AddCategoryModal
        ...
        onCreated={(name: unknown) => setCategories((prev) => [...prev, name as string])}
      />
    </div>
  );
}
