...
const FIELD_GUIDE = [
  ...
  { col: "discount_percentage", req: false, note: "Number 0–100. Leave 0 for no discount." },
  {
    col: "category",
    req: false,
    note: "e.g. Starters, Main Course, Dessert. New categories are created automatically.",
  },
  ...
  { col: "taste_profile", req: false, note: "Savory / Sweet / Spicy / Tangy / Mild / Bitter." },
  {
    col: "ingredients",
    req: false,
    note: 'Comma-separated. Wrap in quotes if any ingredient contains a comma: "Paneer, Masala"',
  },
  ...
  const processFile = async (file) => {
    ...
    try {
      if (ext === "xlsx" || ext === "xls") {
        ...
        if (rows.length === 0) {
          setFileError(
            "No data rows found. Make sure the file has at least one row below the header.",
          );
          return;
        }
        ...
        if (categories.length > 0) {
          ...
          if (badRows.length > 0) {
            const sample = badRows
              .slice(0, 3)
              .map((b) => `row ${b.row} ("${b.cat}")`)
              .join(", ");
            setFileError(
  ...
  return (
    ...
    >
      <div className="space-y-5">
          ...
          <div className="flex items-center justify-between bg-white/5 border border-white/10 rounded-xl px-4 py-3">
            <div>
              ...
              <p className="text-xs text-slate-300 mt-0.5">
                Pre-defined headers — just fill in your data
              </p>
            </div>
            <button
            ...
            <div className="flex items-center justify-between bg-white/5 border border-white/10 rounded-xl px-4 py-3">
              <div>
                ...
                <p className="text-xs text-slate-300 mt-0.5">
                  ...
                  <span className="font-mono text-slate-400">category</span> column must use these
                  exact names
                </p>
              </div>
              <button
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {FIELD_GUIDE.map((f, i) => (
                  <tr
                    key={f.col}
                    className={i < FIELD_GUIDE.length - 1 ? "border-b border-white/5" : ""}
                  >
        ...
        {result && (
          <div
            className={`rounded-xl border px-4 py-4 ${result.imported > 0 ? "bg-green-500/10 border-green-500/20" : "bg-yellow-500/10 border-yellow-500/20"}`}
          >
            {result.imported > 0 ? (
              ...
              <p className="text-sm text-yellow-400">
                No items were imported. Check the file format and try again.
              </p>
            )}
          </div>
        )}
