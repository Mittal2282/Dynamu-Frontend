...
function HeroSection({ tagline, customerName }: HeroSectionProps) {
  return (
...
function ExploreMenuCard({ onClick }: ActionCardProps) {
  return (
    <button
      type="button"
      onClick={onClick}
...
function AskAICard({ onClick }: ActionCardProps) {
  return (
    <button
      type="button"
      onClick={onClick}
...
const MEAL_CONFIG: Record<string, { icon: string; label: string; greeting: string }> = {
...
function TimeBasedSection({ items, mealTime, loading, currencySymbol }: TimeBasedSectionProps) {
...
function TodaysSpecials({ items, loading, currencySymbol }: ItemsSectionProps) {
...
interface NamedItemsSectionProps extends ItemsSectionProps {
  title: string;
}

function ItemsSection({ title, items, loading, currencySymbol }: NamedItemsSectionProps) {
  if (loading) {
    return (
      <div className="pt-6 pb-4 px-4 md:px-6 lg:px-8">
