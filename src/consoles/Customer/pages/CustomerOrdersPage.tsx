...
function IconHourglass({ className, style }: IconProps) {
  return (
    <svg
      className={className}
      style={style}
...
type PhaseIconComponent = React.ComponentType<IconProps>;
...
function OrderBatch({ order, batchIndex, currencySymbol }: OrderBatchProps) {
...
export default function CustomerOrdersPage() {
  const outlet = useOutletContext<CustomerOutletContext | null>();
  const { orderVersion, basePath: baseFromOutlet } = outlet ?? { orderVersion: 0, basePath: "/" };
      ...
      <div className="lg:flex lg:gap-10 lg:items-start">
        ...
        <div className="flex-1 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {original ? (
              <OrderBatch order={original} batchIndex={1} currencySymbol={currencySymbol} />
            ) : null}
