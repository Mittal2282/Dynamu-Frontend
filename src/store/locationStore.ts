...
interface LocationState {
  ...
  _writeCoords: (position: GeolocationPosition) => void;
  _writeError: (err: GeolocationPositionError) => void;
  start: () => void;
  stop: () => void;
  reset: () => void;
  ensureFresh: (maxAgeMs?: number) => Promise<LocationSnapshot>;
}
...
export const locationStore = create<LocationState>((set, get) => ({
  ...
  _writeError: (err) => {
    const next: Partial<LocationState> = { lastError: err };
   ...
   */
  ensureFresh: async (maxAgeMs = 30_000): Promise<LocationSnapshot> => {
    ...
    if (latitude != null && longitude != null && captured_at &&
      ...
      return { latitude, longitude, accuracy_m: accuracy_m!, captured_at };
    }
    try {
      ...
      get()._writeError(err as GeolocationPositionError);
      throw err;
    }
  },
}));
