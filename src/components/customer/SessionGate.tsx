...
interface SessionGateProps {
  qrCodeId: string;
  tableNumber: string;
  onSessionReady: (sessionData: unknown, guestName: string) => void;
}
...
export default function SessionGate({ qrCodeId, tableNumber, onSessionReady }: SessionGateProps) {
  ...
  const [outOfRangeDetails, setOutOfRangeDetails] = useState<OutOfRangeDetails | null>(null);
  ...
  useEffect(() => {
    ...
    const onSessionStarted = (payload: { customer_name?: string; anyone_online?: boolean }) => {
  ...
  const handleStartWithLocation = async (name: string, forceNew: boolean) => {
    try {
