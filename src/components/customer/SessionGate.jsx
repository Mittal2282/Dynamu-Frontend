...
export default function SessionGate({ qrCodeId, tableNumber, onSessionReady }) {
  ...
  const validateName = () => {
    ...
    if (!v) { setNameError('Please enter your name'); return null; }
    if (v.length > 30) { setNameError('Name must be 30 characters or less'); return null; }
    setNameError('');
    return v;
  };
  ...
  if (gateState === 'checking') {
    return card(
      <div className="flex flex-col items-center gap-3 py-4">
        <Spinner size="lg" />
        <Text size="sm" color="muted">Checking table…</Text>
      </div>
    );
  }
  ...
  if (gateState === 'name_entry') {
    return card(<>
      <div className="text-center">
        ...
        <Text size="sm" color="muted" className="mt-1">What's your name?</Text>
      </div>
      <div className="flex flex-col gap-2">
        ...
        />
        {nameError && <Text size="xs" className="text-red-400">{nameError}</Text>}
      </div>
  ...
  if (gateState === 'join_or_create') {
    return card(<>
      ...
      <div className="flex flex-col gap-3">
        ...
        <button
          onClick={() => { validateName() !== null && setGateState('name_entry_new'); }}
  ...
  if (gateState === 'name_entry_new') {
    return card(<>
      <div className="text-center">
        <div className="text-4xl mb-3">🆕</div>
        <Text as="h1" size="lg" weight="bold">New Session</Text>
        <Text size="sm" color="muted" className="mt-1">
  ...
  if (gateState === 'waiting') {
    return card(<>
      <div className="flex flex-col items-center gap-4 py-2 text-center">
        <Spinner size="lg" />
        <div>
          <Text as="h1" size="lg" weight="bold">Waiting for approval</Text>
          <Text size="sm" color="muted" className="mt-1">
            {existingName
        ...
        <div className="w-12 h-12 rounded-full bg-white/5 border border-white/10 flex items-center justify-center">
          <Text size="lg" weight="bold" color="brand">{countdown}</Text>
        </div>
        <button
  ...
  if (gateState === 'rejected') {
    return card(<>
      <div className="flex flex-col items-center gap-4 py-2 text-center">
        ...
        <div>
          <Text as="h1" size="lg" weight="bold">Request Rejected</Text>
          <Text size="sm" color="muted" className="mt-1">
        ...
        <div className="flex flex-col gap-2 w-full">
          ...
          <button
            onClick={() => { setNameInput(''); setGateState('name_entry_new'); }}
            className="text-sm text-slate-500 hover:text-slate-300 transition-colors py-1"
          >
  ...
  if (gateState === 'expired') {
    return card(<>
      <div className="flex flex-col items-center gap-4 py-2 text-center">
        ...
        <div>
          <Text as="h1" size="lg" weight="bold">No Response</Text>
          <Text size="sm" color="muted" className="mt-1">
  ...
  if (gateState === 'error') {
    return card(<>
      <div className="flex flex-col items-center gap-4 py-2 text-center">
        <div className="text-4xl">⚠️</div>
        <div>
          <Text as="h1" size="lg" weight="bold">Something went wrong</Text>
          <Text size="sm" color="muted" className="mt-1">{errorMsg}</Text>
        </div>
        <Button onClick={() => { setGateState('name_entry'); setNameInput(''); }} className="w-full">
          Try again
        </Button>
      </div>
    </>);
