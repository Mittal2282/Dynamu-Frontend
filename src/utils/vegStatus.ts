 ...
 */
export function getItemVegStatus(item: MenuItem): 'veg' | 'nonveg' | 'mixed' {
  const variants: Variant[] = item.has_variants ? (item.variants ?? []) : [];
 ...
 */
export function variantEffectivePrice(variant: Variant): number {
